...
const IMAGES = [];
class ImgComplex {
    constructor({ src, name = src, param, eff = { scx: 1, scy: 1 } }) {
        this.canvs = [];
        this.pixitexs = []; //PIXI.Texture
        IMAGES[name] = this;
...
class Loader {
    static load(callback, draw = null) {
        ...
        function loadNext() {
            if (++n < Loader.list.length)
                Loader.list[n].load(wt, loadNext);
            else
                wt.complete = true;
        }
...
Loader.list = [];
export { Loader, TextureCache, ImgComplex, IMAGES };
